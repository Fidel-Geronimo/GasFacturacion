...
<h1 class="text-center">COBRO DE LOS BONOS</h1>
                ...
                <tbody id="tablaBusqueda">
                    ...
                    while ($row = mysqli_fetch_array($result_facturacion)) { ?>
                        ...
                        if ($repuesta >= 9) {
                            $id = $row['ID'];
                            $credito = "Agotado";
                            $query = "UPDATE bonos set credito = '$credito' WHERE ID = $id";
                    ...
                    <?php
                    $query = "SELECT * FROM bonos WHERE credito = 'Agotado'";
                    $result_facturacion = mysqli_query($conn, $query);
                    $contador = 0;
                            ...
                            <td>
                                ...
                                <div class="collapse" id="<?php echo $azul ?>">
                                    <div class="card card-body">
                                        <a href="Whatsapp/wasaBonos.php?id=<?php echo $row['ID'] ?>" class="btn btn-success botonesOpciones"><i class="fab fa-whatsapp"></i></a>
                                        <a href="abono-bonos.php?id=<?php echo $row['ID'] ?>" class="btn btn-secondary botonesOpciones">Abonar</a>
                                        <a class="btn btn-danger botonesOpciones" onclick="confirmacionPagoBono(<?php echo $row['ID'] ?>)">Pago</a>
                                    </div>
                                </div>
