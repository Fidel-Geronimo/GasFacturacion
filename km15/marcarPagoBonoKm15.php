...
if (isset($_GET["id"])) {
    $id = $_GET['id'];
    // info del cliente
    $queryFactura = "SELECT * FROM bonoskm15 WHERE id = $id";
    ...
    if (mysqli_num_rows($resultadoFactura) == 1) {
        ...
        $queryCliente = "Select * from clientekm15 where id= $idCliente";
        ...
        $queryReset = "DELETE FROM revertirbonoskm15";
        mysqli_query($conn, $queryReset);
        $queryRevertir = "SELECT * FROM bonoskm15 WHERE id= $id";
        ...
        if (mysqli_num_rows($resultadoRevertir) == 1) {
            ...
            $queryRespaldo = "INSERT INTO revertirbonoskm15(id, nombre,fecha, valor,abono,Deuda,credito,comentario,idCliente) VALUES('$id','$nombre', '$fecha','$valor','$abono', '$deuda', '$credito','$comentario','$idCliente')";
            mysqli_query($conn, $queryRespaldo);
        }


        $query = "DELETE FROM bonoskm15 WHERE id = $id";
